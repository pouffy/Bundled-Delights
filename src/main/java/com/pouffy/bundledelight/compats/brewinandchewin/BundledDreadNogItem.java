...
public class BundledDreadNogItem extends BundledBoozeItem {
    ...
    public void affectConsumer(ItemStack stack, Level level, LivingEntity consumer) {
        ...
        if (consumer.hasEffect(BCEffects.TIPSY.get())) {
            ...
            consumer.addEffect(new MobEffectInstance(BCEffects.TIPSY.get(), tipsyEffect.getDuration() + this.duration * 600, tipsyEffect.getAmplifier() + this.potency), consumer);
        } else if (!consumer.hasEffect(BCEffects.TIPSY.get())) {
            consumer.addEffect(new MobEffectInstance(BCEffects.TIPSY.get(), this.duration * 1200, this.potency - 1), consumer);
        }
